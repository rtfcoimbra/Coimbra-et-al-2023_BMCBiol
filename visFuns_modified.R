plotCorRes <- function(
    cor_mat, pop = NULL, ord = NULL, superpop = NULL, title = "Correlation of residuals",
    min_z = NA, max_z = NA, cex.main = 1.5, cex.lab = 1.5, cex.lab.2 = 1.5, cex.legend = 1.5,
    color_palette = c("#001260", "#EAEDE9", "#601200"), pop_labels = c(TRUE, TRUE),
    plot_legend = TRUE, adjlab = 0.1, adjlabsuperpop = 0.16, rotatelabpop = 0,
    rotatelabsuperpop = 0, lineswidth = 1, lineswidthsuperpop = 2
  ){
  ...
  mean_cors <- matrix(
    nrow = length(unique(pop)),
    ncol = length(unique(pop))
  )
  ...
  for(i1 in 1:(length(unique(pop)))){
    for(i2 in 1:(length(unique(pop)))){
      p1 <- unique(pop)[i1]
      p2 <- unique(pop)[i2]
      mean_cors[i1, i2]<- mean(
        cor_mat[which(pop == p1),
  ...
  if(all(is.na(z_lims))){
    z_lims <- c(-max(abs(cor_mat[!is.na(cor_mat)])), max(abs(cor_mat[!is.na(cor_mat)])))
  }

  if(any(is.na(z_lims))){
    z_lims <- c(-z_lims[!is.na(z_lims)], z_lims[!is.na(z_lims)])
  }
  ...
  if(min(cor_mat) < min_z){
    image(ifelse(t(cor_mat < min_z), 1, NA), col = "darkslateblue", add = TRUE)
  }
  ...
  if(pop_labels[2])
    text(
      sort(tapply(1:length(pop), pop, mean) / length(pop)),
      -adjlab,
      adj = c(1, -0.5),
      unique(pop),
      xpd = NA,
  ...
  abline(
    v = grconvertX(
      cumsum(sapply(unique(pop), function(x){sum(pop == x)})) / N,
      "npc",
      "user"
    ),
    col = 1,
  ...
  if(!is.null(superpop)){
    ...
    if(pop_labels[2])
      text(
        sort(tapply(1:length(superpop), superpop, mean) / length(superpop)),
        -adjlabsuperpop,
        unique(superpop),
        xpd = NA,
        cex = cex.lab.2,
        srt = rotatelabsuperpop,
        font = 2
      )
    if(pop_labels[1])
      text(
        ...
        xpd = NA,
        cex = cex.lab.2,
        srt = 90 - rotatelabsuperpop,
        font=2
      )
    abline(
      v = grconvertX(
        cumsum(sapply(unique(superpop), function(x){sum(superpop == x)})) / N,
        "npc",
        "user"
      ),
      col = 1,
      lwd = lineswidthsuperpop,
      xpd = FALSE
    )
    abline(
      h = grconvertY(
  ...
  if(plot_legend){
    ...
    rasterImage(rlegend, 0, 0.25, 0.3, 0.75)
    text(
      x = 0.5,
      y = c(0.25, 0.5, 0.75),
